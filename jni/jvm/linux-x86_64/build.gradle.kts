 ...
 * Ships libsdl_image_jni.so as a classpath resource at
 * /cn/enaium/sdl/image/native/linux-x86_64/, which ImageNativeLoader
...
val libFile = "libsdl_image_jni.so"
...
val canBuildHere = hostIsLinuxX64
...
val configureJniLibrary by tasks.registering(Exec::class) {
    group = "build"
    description = "cmake-configures libsdl_image_jni for $classifier."
    onlyIf { canBuildHere }
...
val buildJniLibrary by tasks.registering(Exec::class) {
    group = "build"
    description = "Builds libsdl_image_jni.so for $classifier."
    onlyIf { canBuildHere }
